...
$video_nav = '<div class="video-label">PLAY VIDEOS:</div>'; //video gallery navigation
$cont = 0;
...
while($node->field_event_video_gallery[$cont]['value']){
	...
	if(empty($image_output) && $cont==0) $video_output .= '<div class="event-slide event-show" name="video-slide'.$cont.'"><iframe width="703" height="399" src="'.$new_video_url.'?rel=0" frameborder="0" allowfullscreen></iframe></div>';
	...
	if($node->field_event_video_gallery[1]['value']) $video_nav .= '<div name="video-slide'.$cont.'" class="event-nav">'.($cont+1).'</div>';
		...
		else $video_nav = '<div name="video-slide'.$cont.'" class="event-nav video-unique">PLAY VIDEO</div>';
...
<div id="event-slideshow" class="<?php print $hidden ?>">
	...
	<div id="event-slideshow-nav" class="<?php print $hidden ?>">
		...
		<?php if($node->field_event_audio[0]['filepath']): ?>
			<div id="event-audio">
				<object type="application/x-shockwave-flash" data="<?php print path_to_theme() ?>/js/dewplayer-rect.swf" width="240" height="20" id="dewplayer" name="dewplayer">
					<param name="wmode" value="transparent" />
					<param name="movie" value="<?php print path_to_theme() ?>/js/dewplayer-rect.swf" />
					<?php
						if(count($node->field_event_audio)>1){
							$size = count($node->field_event_audio)-1;
							for($i=0; $i<count($node->field_event_audio); $i++){
								if($i==$size) $tracks .= $node->field_event_audio[$i]['filepath'];
									else $tracks .= $node->field_event_audio[$i]['filepath'].'|';
							}
						} else $tracks = $node->field_event_audio[0]['filepath'];
					?>
					<param name="flashvars" value="mp3=<?php print $tracks ?>&amp;autoreplay=1" />
				</object>
			</div>
		<?php endif; ?>
	</div>
...
<div id="event-body">
	...
	<div id="event-date">
		<?php
			$size = count($node->field_event_date);
			
			if($size==1){
				$date_from = substr($node->field_event_date[0]['value'], 0, 10);
				$date_to = substr($node->field_event_date[0]['value2'], 0, 10);
				
				if($value['value2'] && ($date_to > $date_from) ) print '<div><span>'.date('F j, Y',strtotime($date_from)).'-'.date('F j, Y',strtotime($date_to)).'</span></div>';
					else print '<div><span>'.date('F j, Y',strtotime($date_from)).'</span></div>';
			}
			else{
				$date_from = substr($node->field_event_date[0]['value'], 0, 10);
				$date_to = substr($node->field_event_date[$size-1]['value'], 0, 10);
